    ...
    <v-container>
        <v-layout row wrap>
            <v-flex d-flex xs12 mt-5 offset-lg2>
                <h1 class="display-3 font-weight-thin mb-3">Caronas oferecidas</h1>
            </v-flex>
            ...
            <template v-if="!hasScope">
                <v-flex d-flex xs12 md8 mt-5 offset-md2>
                    <v-alert value="true" type="error" >
                        <b>Algumas permissões estão faltando</b>. Por favor, refaça o login e aceite pelo menos as permissões de informações sobre suas caronas e para edição de suas caronas.
                    </v-alert>
                </v-flex>
                <v-flex
                d-flex
            ...
            <template v-else-if="!$auth.user.driver">
                <v-flex d-flex xs12 md8 mt-5 offset-md2>
                    <v-alert value="true" type="error" >
                        <b>Você não é motorista!</b> Para ver caronas criadas por você é necessário preencher seus dados como motorista.
                    </v-alert>
                </v-flex>
                <v-flex
                d-flex
...
<script>
import List from '~/components/driver/List.vue'
import TripDialog from '~/components/driver/TripDialog.vue'
import TripStats from '~/components/driver/TripStats.vue'
...
export default {
    ...
    head() {
        return {
            title: 'Caronas oferecidas - ' + process.env.APP_NAME
        }
    },
    mounted() {
        this.hasScope =
        ...
        if (this.hasScope && this.$auth.user.driver) {
            this.getOld()
            this.getCurrent()
        }
    },
    methods: {
        async getOld() {
            ...
            let endpoint = API_URL + '/trips/driver/'
            let payload = {
                method: 'get',
                url: endpoint,
                params: {
